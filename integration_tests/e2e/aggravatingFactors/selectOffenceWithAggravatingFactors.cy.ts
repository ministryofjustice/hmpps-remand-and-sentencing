...
context('Select offence with aggravating factors Page', () => {
  ...
  context('when all offence has sentence', () => {
    beforeEach(() => {
      ...
      // NOTE: do NOT create a dismissed/non-sentencing offence for this scenario
    ...
    it('page should show error if none of the offence is selected', () => {
      ...
      selectOffenceWithAggravatingFactorsPage
        .errorSummary()
        .trimTextContent()
        .should('equal', 'There is a problem Select at least one offence to apply aggravating factors to')
    })
  ...
  context('when there are non sentencing offence', () => {
    beforeEach(() => {
      cy.task('happyPathStubs')
      cy.task('stubGetSentenceTypeById', {})
      cy.task('stubGetOffenceByCode', {})
    ...
    it('only offence with sentence should show up', () => {
      ...
      selectOffenceWithAggravatingFactorsPage = new AggravatingFactorsSelectOffenceWithAggravatedFactorsPage()
      selectOffenceWithAggravatingFactorsPage.aggravatedOffenceCheckboxes().should('have.length', 1)
    })
  })
})
