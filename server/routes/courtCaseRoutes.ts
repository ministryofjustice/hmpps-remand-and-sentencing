...
export default class CourtCaseRoutes {
  ...
  public submitTaskListAsDraft: RequestHandler = async (req, res): Promise<void> => {
    ...
    if (addOrEditCourtCase === 'add-court-case' && !courtAppearance.existingDraft) {
      const courtCase = { appearances: [courtAppearance] } as CourtCase
      await this.remandAndSentencingService.createDraftCourtCase(username, nomsId, courtCase, req.body, draftSubmittedFrom)
    } else {
      await this.createOrUpdateDraftAppearance(username, courtCaseReference, courtAppearance, req.body, draftSubmittedFrom)
    }
    return res.redirect(
