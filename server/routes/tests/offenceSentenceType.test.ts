...
describe('GET Offence Sentence Type', () => {
  ...
  it('should render error when no sentence types returned', async () => {
    ...
    expect(errorSummary).toContain(
      'There are no eligible sentence types to display for the selected outcome based on the offender and offence details.  Check the details entered and try again.',
    )
  })
})
