...
describe('AppearanceOutcomeService', () => {
  ...
  let service: AppearanceOutcomeService
  ...
  beforeEach(() => {
    ...
    service = new AppearanceOutcomeService(remandAndSentencingApiClient)
  })

  describe('getOutcomeByUuid', () => {
    it('should call the api if the cached outcomes dont contain the outcome (e.g. if its inactive)', async () => {
      ...
      const result = await service.getOutcomeByUuid(inactiveOutcomeUuid, 'user')
