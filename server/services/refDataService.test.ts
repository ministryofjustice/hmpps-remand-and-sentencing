...
describe('RefDataService', () => {
  ...
  describe('OffenceOutcomeTests', () => {
    it('should call the api if the cached outcomes dont contain the outcome (e.g. if its inactive)', async () => {
      ...
      const result = await service.getChargeOutcomeById(inactiveOutcomeUuid, 'user')
      expect(result).toEqual(apiResult)
    })
    ...
    it('returns mapped outcomes from cache when all IDs are present', async () => {
      ...
      const result = await service.getChargeOutcomeMap(outcomeIds, username)
